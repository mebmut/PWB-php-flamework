...
class Database {
  ...
  private function __construct() {
    $db = getJsonFile(ROOT.'app'.DS.'settings'.DS.'database.json');
    if ($db===null) {
      throw new Exception("Sorry the database is not set", 1);
    }
    try {
      $this->_pdo = new PDO('mysql:host='.$db->dbhost.';dbname='.$db->dbname, $db->dbuser, $db->dbpass);
